/*
    IDE Controller Driver Implementation
    Written by AHK Developer(s): <Advanced/Adaptable Hybrid Kernel Team>
    See OSDEV Wiki ATA_PIO Mode for a in-depth overview
*/
...
uint8 ide_print_error(uint32 drive, uint8 err) {
    ...
    if (err == 1) {
        ...
        if (st & ATA_ER_TK0NF) {
            monitor_write("- No Media or Media Error\n");
            err = 3;
        }
        if (st & ATA_ER_ABRT) {
    ...
    monitor_write(g_ide_devices[drive].model);
...
void ide_init(uint32 prim_channel_base_addr, uint32 prim_channel_control_base_addr,
    ...
    for (i = 0; i < 4; i++)
        if (g_ide_devices[i].reserved == 1) {
            monitor_write_dec(i);
            monitor_write("\n  model: ");
            monitor_write(g_ide_devices[i].model);
            monitor_write("\n");
        }
}
...
int ide_read_sectors(uint8 drive, uint8 num_sectors, uint32 lba, uint32 buffer) {
    ...
    if (drive > MAXIMUM_IDE_DEVICES || g_ide_devices[drive].reserved == 0) {
        monitor_write("IDE ERROR: Drive not found\n");
        return -1;
    }
